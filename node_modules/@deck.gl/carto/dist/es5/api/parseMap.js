...
function createChannelProps(visualChannels, type, config, data) {
  ...
  if (colorField) {
    result.getFillColor = (0, _layerMap.getColorAccessor)(colorField, colorScale, visConfig.colorRange, 1, data);
  }

  if (strokeColorField) {
    var opacity = visConfig.strokeOpacity !== undefined ? visConfig.strokeOpacity : 1;
    result.getLineColor = (0, _layerMap.getColorAccessor)(strokeColorField, strokeColorScale, visConfig.strokeColorRange, opacity, data);
  }

  if (heightField) {
    result.getElevation = (0, _layerMap.getSizeAccessor)(heightField, heightScale, visConfig.heightRange || visConfig.sizeRange, data);
  }

  if (sizeField) {
    result.getPointRadius = (0, _layerMap.getSizeAccessor)(sizeField, sizeScale, visConfig.radiusRange || visConfig.sizeRange, data);
  }
